...
using static ЛР_2.OSType;
using static ЛР_2.ProcessorType;
...
namespace ЛР_2
{
    class Computer : IComputer, IOverlock
    {
        ...
        // ЗАДАНИЕ 3 ИЗ ЛАБОРАТОРНОЙ 3

        public delegate void Delegate();

        event Delegate addNewUser = null;
        event Delegate changeProcessor = null;
        event Delegate installNewSoftware = null;
        event Delegate changeRAM = null;

        public event Delegate AddNewUser
        {
            add
            {
                if (!value.Method.Name.Contains("remove"))
                    addNewUser += value;
            }
            remove
            {
        ...
        public event Delegate ChangeProcessor
        {
            add
            {
        ...
        public event Delegate InstallNewSoftware
        {
            add
            {
                installNewSoftware += value;
            }
            remove
            {
                installNewSoftware -= value;
        ...
        public event Delegate ChangeRAM
        {
            add
            {
                changeRAM += value;
        ...
        {
            addNewUser.Invoke();
            changeProcessor.Invoke();
            installNewSoftware.Invoke();
            changeRAM.Invoke();
        }
